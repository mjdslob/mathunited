...
<xsl:stylesheet version="1.0"
   ...
   <xsl:variable name="subcomponent" select="$parsed_component/component/subcomponents/subcomponent[@id=$subcomp]"/>
    ...
    <xsl:variable name="urlbase">
        <xsl:value-of select="concat('http://mathunited.pragma-ade.nl:41080/data/',$refbase)"/>
    </xsl:variable>
    ...
    <xsl:variable name="overviewRef">
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:value-of select="string('http://www.wageningse-methode.nl/methode/het-lesmateriaal/')"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    <!--   /////////////////////////////////////////////   -->
    ...
    <xsl:template match="exercise" mode="numbering">
        <xsl:copy>
            <xsl:attribute name="num">
                <xsl:value-of select="1+count(preceding::exercise)"/>
            </xsl:attribute>
                ...
                <xsl:choose>
                    <xsl:when test="string-length($ws_id) gt 0">
                        <div class="pageDiv-for-worksheet">
                            <div class="worksheet-title">Werkblad bij <xsl:value-of select="$component_title"/> &gt; 
                    ...
                    <xsl:otherwise>
                        <div class="pageDiv">
                            ...
                            <div class="section-container">
                                <xsl:attribute name="_base">
                                    <xsl:value-of select="subcomponent/@_base"/>
                                </xsl:attribute>
                                <xsl:apply-templates select="subcomponent/section"/>
                            </div>
                            <xsl:variable name="logoUrl">
                                <xsl:choose>
                                    <xsl:when test="$host_type='GAE'">/sources_wm/vo-content_logo_stercollectie_small.jpg</xsl:when>
                                    <xsl:otherwise>sources_wm/vo-content_logo_stercollectie_small.jpg</xsl:otherwise>
                                </xsl:choose>
                            </xsl:variable>
                            <div id="vo-content-logo-container">
                                <img src="{$logoUrl}"/>
                            </div>
                        </div>
                        <xsl:if test="count(subcomponent//exercise)>0">
                            <xsl:variable name="vraagtekenUrl">
                                <xsl:choose>
                                    <xsl:when test="$host_type='GAE'">/sources_wm/Vraagteken.png</xsl:when>
                                    <xsl:otherwise>sources_wm/Vraagteken.png</xsl:otherwise>
                                </xsl:choose>
                            </xsl:variable>
                            <div class="menuDiv">
    ...
    <xsl:template match="subsection" mode="content">
        <div class="section-container container_12 clearfix">
            ...
            <div class="subsection-title grid_11 prefix_1">
    ...
    <xsl:template match="exercise" mode="content">
        ...
        <div id="ex-{@num}" num="{@num}" refid="{@id}">
            <xsl:choose>
                <xsl:when test="@wm_show='disablefalse'">
            ...
            <xsl:otherwise>
                ...
                <xsl:choose>
                    <xsl:when test="$_cross_ref_as_links_">
                        <a>
                            <xsl:attribute name="href">
                                <xsl:value-of select="concat('view?comp=',$_comp,'&amp;subcomp=',$_subcomp,'&amp;variant=',$variant,'&amp;id=', @item)"/>
                            </xsl:attribute>
                            <xsl:value-of select="."/>
