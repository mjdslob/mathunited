...
tinymce.PluginManager.add('m4a_image', function(editor) {
	function showDialog() {
		...
		if (imgElm.nodeName == 'IMG' && !imgElm.getAttribute('data-mce-object')) {
			data = {
				...
				width: width,
				height: height,
                                location: dom.getAttrib(imgElm, 'location')
			};
		} else {
			imgElm = null;
		}
		...
		var generalFormItems = [
			...
			imageListCtrl,
			{name: 'location', type: 'listbox', values: [{text:'op deze plek',value:'here'},{text:'rechts uitgelijnd',value:'right'}], label: 'location'},
