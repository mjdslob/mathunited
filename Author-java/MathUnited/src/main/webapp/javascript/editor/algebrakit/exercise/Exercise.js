...
define(["require", "akit/Engine", "exercise/ExerciseItem"], function (require, Engine, ExerciseItem) {
    //spec:
    // - id 
    // - dom : $('.akit-exercise')
    // - onFinished : callback 
    function Exercise(spec) {
        ...
        if (paletteVariableSpec) {
            ...
            if (paletteVariableSpec.length > 2)
                paletteVariableSpec = [paletteVariableSpec[0], paletteVariableSpec[1]];
            else if (paletteVariableSpec.length === 1)
                paletteVariableSpec = [paletteVariableSpec[0], 'y'];
        } else {
        ...
        var obj = {
            ...
            init: function () {
                ...
                _this.executeScript(dom.children('.akit-script').first(), function () {
                    if (_this.dom.hasClass('akit-init-open')) {
                        create();
                    }
                    else
            ...
            showItem: function (spec) {
                ...
                if (this.activeItem && this.activeItem.id !== spec.itemId)
                ...
                if (item) {
                    ...
                    if (spec.relativeItem && this.activeItem) {
                        if (spec.placeBefore)
                            this.activeItem.dom.before(item_jq);
                        else
                            this.activeItem.dom.after(item_jq);
                    }
                    item = new ExerciseItem({
                        ...
                        whenDone: function (v) {
                            ...
                            if (s) {
                                ...
                                setTimeout(function () {
                                    if (s)
                                        _this.showItem({itemId: s, relativeItem: _this.activeItem, placeBefore: false, doAnimate: true});
                                }, 1000);
                            } else {
