...
define(["require", "akit/Engine", "akit/Derivation", "exercise/Palette", "akit/Util",
    "exercise/Cell", "jqueryui"], function (require, Engine, Derivation, Palette, Util, Cell, jqueryUI) {
        ...
        var obj = {
            ...
            init: function () {
                ...
                if (obj.status === 'uninitialized') {
                    ...
                    dom.append(tool_jq);
                    if(this.label) {
                        $('.akit-input-label',tool_jq).html(this.label).css('display', 'inline-block');
                    }
                ...
                function callback(result) {
                    ...
                    if (result.exerciseStatus === 'NO_MATCH') {
                        ...
                        if (result.exerciseStatus === 'FINISHED') {
                            ...
                            if (result.derivationStatus === 'FINISHED')
                                obj.setActiveCell(rootCell);
                            else
                                obj.setActiveCell(cells[0]);
                            obj.clearInputField();
                        }
                    }
                }
            ...
            getHintId: function () {
                if (dom.attr('hint'))
                    return dom.attr('hint');
            },
            getHint: function (level) {
                if (this.finished || !this.showHints)
                    return;
                if (!level)
                    level = 1;
            ...
            setActiveCell: function (cell) {
                if (activeCell)
                    activeCell.dom.removeClass('active');
                activeCell = cell;
                if (cell)
