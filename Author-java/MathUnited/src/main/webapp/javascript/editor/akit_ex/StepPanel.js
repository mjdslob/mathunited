...
define(['jquery', 'akitex/Parser'], function ($, AKITParser) {
...
//parentStep: StepPanel object that contains this step (used for afterstep processing)
    ...
    StepPanel.prototype.createButtons = function () {
        if (this.outDiv) {
            ...
            this.explainButton.onclick = function () {
                thisStepPanel.showExplanation();
            }
            this.explainButton.onmouseover = function () {
                this.className = 'ExplainButtonHover';
            }
            this.explainButton.onmouseout = function () {
                this.className = 'ExplainButton';
            }
            this.explainButton.onmousedown = function () {
                this.className = 'ExplainButtonDown';
            }
            this.explainButton.onmouseup = function () {
                this.className = 'ExplainButtonHover';
            }
            ...
            this.collapseButton.onclick = function () {
                thisStepPanel.collapseExplanation();
            }
            this.collapseButton.onmouseover = function () {
                this.className = 'CollapseButtonHover';
            }
            this.collapseButton.onmouseout = function () {
                this.className = 'CollapseButton';
            }
            this.collapseButton.onmousedown = function () {
                this.className = 'CollapseButtonDown';
            }
            this.collapseButton.onmouseup = function () {
                this.className = 'CollapseButtonHover';
            }
            ...
            this.collapseInnerButton.onmouseover = function () {
                this.className = 'CollapseInnerButtonHover';
            }
            this.collapseInnerButton.onmouseout = function () {
                this.className = 'CollapseInnerButton';
            }
            this.collapseInnerButton.onmousedown = function () {
                this.className = 'CollapseInnerButtonDown';
            }
            this.collapseInnerButton.onmouseup = function () {
                this.className = 'CollapseInnerButtonHover';
            }
            this.collapseInnerButton.onclick = function () {
                thisStepPanel.collapseAfterSteps();
            }
            this.expandButton = document.createElement('div');
            this.expandButton.className = 'ExpandButton';
            this.expandButton.onmouseover = function () {
                this.className = 'ExpandButtonHover';
            }
            this.expandButton.onmouseout = function () {
                this.className = 'ExpandButton';
            }
            this.expandButton.onmousedown = function () {
                this.className = 'ExpandButtonDown';
            }
            this.expandButton.onmouseup = function () {
                this.className = 'ExpandButtonHover';
            }
            this.expandButton.onclick = function () {
                thisStepPanel.showAfterSteps();
            }
...
//    var elms = this.subDiv.getElementsByClassName('subProcStep');
        ...
        if (elms.length > 0)
            new StepPanel(procArray[ii], elms[ii]);
                ...
                case "stepList":
                    ...
                    for (var ii = 0; ii < elm.childNodes.length; ii++) {
                        if (elm.childNodes[ii].nodeType == 1)
                            stepList.push(AKIT_ParseStepXML(elm.childNodes[ii]));
                    }
                    break;
                case "afterList":
                    afterList = [];
                    for (var ii = 0; ii < elm.childNodes.length; ii++) {
                        if (elm.childNodes[ii].nodeType == 1)
                            afterList.push(AKIT_ParseStepXML(elm.childNodes[ii]));
                    }
                    break;
                case "text":
                    text = elm;
