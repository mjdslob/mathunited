...
<xsl:stylesheet version="2.0"
        ...
        <xsl:choose>
            <xsl:when test="@prevent-am">
                <span class="math-container">
                    <span tag="m:math">
                        <xsl:apply-templates mode="paragraph-span"/>
                    </span>
    ...
    <xsl:template match="sheetref" mode="editor">
        <span tag="sheetref"><xsl:apply-templates select="@* | node()" mode="editor"/>&#160;
            <span>
    ...
    <!-- Special treatment for paragraphs of bron-clas -->
    <xsl:template match="p[@class='bron']" mode="paragraph">
        <div tag="bron">
            <xsl:apply-templates mode="paragraph"/>
        </div>
    </xsl:template>
    ...
    <xsl:template match="resourcelink" mode="paragraph">
        <a target="_blank"
           class="dox"> <!-- @class='dox' is used in GenerateQTI to find these resourcelink, do not change -->
            <xsl:for-each select="@*">
                <xsl:choose>
                    <!-- relative url w.r.t. base path of content -->
                    <xsl:when test="name()='href'">
                        <xsl:attribute name="href">
                            <xsl:value-of select="concat($urlbase,'../dox/',.)"/>
                        </xsl:attribute>
                    </xsl:when>
                    <xsl:otherwise>
