        ...
        <xsl:when test="$is_mobile!='DISABLEtrue'">
            ...
            <iframe style="overflow:hidden" _type='ggb'>
                ...
                <xsl:choose>
                   ...
                   <xsl:otherwise>
		       <xsl:attribute name="src"><xsl:value-of select="concat('/MathUnited/geogebra?file=',$refbase,replace(replace(@filename,'geogebra/','../geogebra/'),'GeoGebra/','../geogebra/'),'&amp;repo=',$repo)"/></xsl:attribute>
                   </xsl:otherwise>
                </xsl:choose>
            </iframe>
        </xsl:when>
